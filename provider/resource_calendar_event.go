...
func resourceCalendarEventRead(d *schema.ResourceData, meta interface{}) error {
	...
	if event.Reminders != nil && len(event.Reminders.Overrides) > 0 {
		d.Set("reminder", flattenCalendarEventReminders(event.Reminders.Overrides))
	}

	// Handle attendees
	if len(event.Attendees) > 0 {
		d.Set("attendee", flattenCalendarEventAttendees(event.Attendees))
	}
...
// resourceCalendarBuildEvent is a shared helper function which builds an "event" struct
// from the schema. This is used by create and update.
func resourceCalendarEventBuild(d *schema.ResourceData, meta interface{}) (*calendar.Event, error) {
...
func flattenCalendarEventAttendees(list []*calendar.EventAttendee) []map[string]interface{} {
...
// storing in the schema.
func flattenCalendarEventReminders(list []*calendar.EventReminder) []map[string]interface{} {
	result := make([]map[string]interface{}, len(list))
	for i, v := range list {
		result[i] = map[string]interface{}{
			"method": v.Method,
